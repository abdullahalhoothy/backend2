...
@pytest.mark.asyncio
async def test_change_email(async_client , req_auth_user_data, firebase_sign_in):
    data_payload = {
        "message": "string",
        "request_info": {},
        "request_body": {
                "user_id": "sUEGmzrKaOW3hBcaOKM73HJLTRa2",
                "current_email": "shairf@gmail.com",
                "new_email": "nosharif@gmail.com",
                "password": "string"
        }
    }
    mock_user = MagicMock(**{
    ...
    with (patch("backend_common.auth.make_firebase_api_request", new_callable=AsyncMock) as firebase_data,
          patch.object(auth, "get_user", return_value=mock_user) as mock_get_user
           ...
           response = await async_client.post("/fastapi/change_email" , json=data_payload)
           assert response.status_code == 200


@pytest.mark.asyncio
async def test_change_email_userid_mismatch(async_client , req_auth_user_data, firebase_sign_in):
    data_payload = {
        "message": "string",
        "request_info": {},
        "request_body": {
                "user_id": "sUEGmzrKaOW3hBc",
                "current_email": "shairf@gmail.com",
                "new_email": "nosharif@gmail.com",
                "password": "string"
        }
